...
class MessageSubscriber(Node):
    ...
    def listener_callback(self, msg):
        """
        This function is called every time a message is published to the 'input' topic.
        ...
        response_msg = String()
        # Placeholder to test
        # If data = right, 'send current' to right motor, will implement actual sending once I know how to
        if(data.lower() == "right"):
            response_msg.data = 'Sendin current to right motor'
        else:
            response_msg.data = f'Unknown message received: {msg.data}'

        self.publisher.publish(response_msg)
        self.get_logger().info(f'Publishing to /output: "{response_msg.data}"')
